...
public class MediaPlayback extends Activity implements View.OnTouchListener,
	...
	public void update(Observable observable, Object data) {
		...
		if (message == Downloader.FINISHED_DOWNLOAD.intValue()) {
			...
			Toast tst1 = Toast.makeText(MediaPlayback.this,
					...
					Toast.LENGTH_LONG);
			tst1.setGravity(Gravity.CENTER, tst1.getXOffset() / 2,
					tst1.getYOffset() / 2);
			tst1.show();
			stopNotification();
			Contents.clearState();
	...
	public void startNotification() {
		...
		notification.setLatestEventInfo(getApplicationContext(),
				Contents.getSong().name, Contents.getSong().artist,
				contentIntent);
		...
		case MENU_DOWNLOAD:
			if (Downloader.isSDPresent() && Contents.mediaPlayer != null
				...
				try {
					...
					handler.removeMessages(REFRESH);
					Contents.mediaPlayer.pause();
					DownloadListener.copyFile(Contents.currentlyPlayingFile,
							destination);
					if (isPlaying)
						Contents.mediaPlayer.start();
					queueNextRefresh(refreshNow());
					Toast tst = Toast.makeText(MediaPlayback.this,
							getString(R.string.save_complete) + destination,
							Toast.LENGTH_LONG);
					...
					queueNextRefresh(refreshNow());
					Toast tst = Toast.makeText(MediaPlayback.this,
							getString(R.string.error_title), Toast.LENGTH_LONG);
					tst.setGravity(Gravity.CENTER, tst.getXOffset() / 2,
							tst.getYOffset() / 2);
					tst.show();
					e.printStackTrace();
