...
abstract class DNSRecord extends DNSEntry {
   ...
   static class Address extends DNSRecord {
      ...
      boolean handleQuery(JmDNS dns, long expirationTime) {
         ...
         if (dnsAddress != null) {
            if (dnsAddress.sameType(this) && dnsAddress.sameName(this)
               ...
               logger
                     .finer("handleQuery() Conflicting probe detected. dns state "
                           + dns.getState()
                           + " lex compare "
               ...
               if (dns.getState().isProbing() && lexCompare(dnsAddress) >= 0) {
                  ...
                  dns.getCache().clear();
                  for (Iterator i = dns.services.values().iterator(); i
                        .hasNext();) {
      ...
      DNSOutgoing addAnswer(JmDNS dns, DNSIncoming in, InetAddress addr,
            int port, DNSOutgoing out) throws IOException {
         return out;
      }
      ...
      boolean handleQuery(JmDNS dns, long expirationTime) {
         ...
         if (info != null
            ...
            if (info.getState().isProbing()
               ...
               logger
                     .finer("handleQuery() Lost tie break: new unique name chosen:"
                           + info.getName());
      ...
      DNSOutgoing addAnswer(JmDNS dns, DNSIncoming in, InetAddress addr,
         ...
         if (info != null) {
            if (this.port == info.port != server.equals(dns.getLocalHost()
                  .getName())) {
               return dns.addAnswer(in, addr, port, out, new DNSRecord.Service(
